...
import top.e404.warp.warp.Warp
    ...
    override fun onCommand(
        ...
        when (args.size) {
            2 -> {
                ...
                if (warp != null) {
                    p.sendMsgWithPrefix(
                        if (!warp.isOwner(p)) "&c此名字的warp已存在"
                ...
                val l = p.location
                WarpManager.warps[name] = Warp(
                    name, p.name, create, true, 0.0, p.world.name,
            ...
            6 -> {
                ...
                when {
                    ...
                    sz == null -> sender.sendMsgWithPrefix("&cZ的数值错误")
                        ...
                        WarpManager.warps[name] = Warp(
                            name, sender.name, WarpManager.sdf.format(Date()),
